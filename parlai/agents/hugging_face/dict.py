...
try:
    from transformers import GPT2Tokenizer, GPT2TokenizerFast, T5TokenizerFast, MT5TokenizerFast, BloomTokenizerFast
except ImportError:
    raise ImportError(
...
SPECIAL_TOKENS_GPT2B = {"bos_token": "<s>", "eos_token": "</s>", "pad_token": "<pad>"}
BLOOM_DEFAULT_MODEL_NAME = "bigscience/bloom-560m"
...
class Gpt2bDictionaryAgent(HuggingFaceDictionaryAgent):
    ...
    def _define_special_tokens(self, opt):
        if opt["add_special_tokens"]:
            # Add additional start/end/pad tokens
            self.hf_tokenizer.add_special_tokens(SPECIAL_TOKENS_GPT2B)
            self.start_token = SPECIAL_TOKENS_GPT2B["bos_token"]
            self.end_token = SPECIAL_TOKENS_GPT2B["eos_token"]
            self.null_token = SPECIAL_TOKENS_GPT2B["pad_token"]
        else:
            # Only special token is end of text
            self.start_token = NO_OP  # hack, we cut off the start token
            self.end_token = "<|endoftext|>"
...
class BloomDictionaryAgent(HuggingFaceDictionaryAgent):
    @property
    def add_special_tokens(self) -> bool:
        """
        Whether to add special tokens when tokenizing.
    ...
    def get_tokenizer(self, opt):
        ...
        if opt.get("model_name"):
            ...
            fle_key = BLOOM_DEFAULT_MODEL_NAME
        return BloomTokenizerFast.from_pretrained(fle_key)
